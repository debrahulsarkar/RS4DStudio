    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: `New RS4DStudio project inquiry from ${values.name}`,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-[8px] border border-white/10 bg-studio-card p-5 shadow-card sm:p-7" noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Project Type" error={errors.projectType?.message}>
          <select {...register("projectType")} className={cn(inputClass, "appearance-none")} defaultValue="">
            <option value="" disabled>Choose one</option>
            <option>Collectible statue</option>
            <option>Miniature sculpt</option>
            <option>Game character</option>
            <option>3D print preparation</option>
            <option>Custom statue</option>
          </select>
        </Field>
        ...
        <Field label="Message" error={errors.message?.message} className="sm:col-span-2">
          <textarea {...register("message")} className={cn(inputClass, "min-h-36 resize-y")} placeholder="Tell me about the character, scale, deadline, references, and final use." />
        </Field>
      </div>
      <button type="submit" disabled={isSubmitting} className="focus-ring mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-studio-accent px-5 py-4 text-sm font-bold text-black shadow-glow transition hover:-translate-y-0.5 hover:bg-orange-400 disabled:cursor-wait disabled:opacity-70 sm:w-auto">
        {isSubmitting ? "Sending..." : "Send Project Brief"} <Send size={16} />
      </button>
